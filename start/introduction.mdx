---
title: "平台介绍"
description: "FCLOUD API · 一站式大模型开发平台"
---
<Frame>
  <img className="block dark:hidden img-h25" src="https://download.fcloud.net/directlink/2/api-docs/logo-dark.svg" alt="节点跃迁AI开放平台" />

  <img className="hidden dark:block img-h25" src="https://download.fcloud.net/directlink/2/api-docs/logo-light.svg" alt="节点跃迁AI开放平台" />
</Frame>

## 平台定位

节点跃迁大模型开放平台 [fcloud.net](https://www.fcloud.net)，提供功能丰富、灵活易用、高性价比的大模型 API 服务，支持智能体开发与模型精调、推理、评测等，致力于构建高效通用的 “一站式模型即服务” AI 开发新范式。
...
您可前往节点跃迁大模型 [操练场](https://www.fcloud.net/console/playground)，极速体验模型能力。

<Tip>
  在对话框输入相关参数及prompt，点击发送，即可看到对应模型所输出的结果。

  ![](https://download.fcloud.net/directlink/2/api-docs/ScreenShot_2026-02-06_162841_085.png)
...
[快速开始](/start/quick-start) 将引导您逐步完成 API 调用流程，涵盖注册账号、环境配置、获取 API key、SDK 使用等关键步骤。帮助您分钟级完成模型调用服务，并集成到您的业务或应用中。
...
<Tabs>
  <Tab title="Token">
    <Card title="Token - 文本处理单位" icon={<svg style={{maskImage: "url(/resource/icon/coins.svg)", maskRepeat: "no-repeat", maskPosition: "center center",}} className={"h-6 w-6 bg-primary dark:bg-primary-light !m-0 shrink-0"}/>}>
      ...
      大多数模型中 token 和字数的换算比例约为 1:1.6 - 2 ，但因为不同模型的分词不同，所以换算比例也存在差异，每一次实际处理 token 数量以模型返回为准，您可以从返回结果的 usage 中查看。
    </Card>
  </Tab>
  ...
  <Tab title="上下文窗口">
    <Card title="Context Window - 上下文窗口" icon={<svg style={{maskImage: "url(/resource/icon/window.svg)", maskRepeat: "no-repeat", maskPosition: "center center",}} className={"h-6 w-6 bg-primary dark:bg-primary-light !m-0 shrink-0"}/>}>
      ...
      * 用户输入的内容
      * 模型生成的回复
      * 模型在生成回复过程中进行推理或调用工具时产生的中间内容（如：claude-opus-4-5-thinking ）
